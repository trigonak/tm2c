...
#ifndef _SYS_NIAGARA_H_
#define _SYS_NIAGARA_H_
...
{
  TM2C_RPC_STATS_T* tm2c_rpc_stats = (TM2C_RPC_STATS_T*) data;
  TM2C_RPC_STATS_SEND_T tm2c_rpc_stats_send;
  tm2c_rpc_stats_send.type = TM2C_RPC_STATS;
  tm2c_rpc_stats_send.nodeId = NODE_ID();

  data = (void*) &tm2c_rpc_stats_send;
  if (tm2c_rpc_stats->tx_duration)
    {
      tm2c_rpc_stats_send.stats_type = 0;
      tm2c_rpc_stats_send.tx_duration = tm2c_rpc_stats->tx_duration;
      int target;
      for (target = 0; target < NUM_DSL_NODES; target++) 
	{
      ...
      tm2c_rpc_stats_send.stats_type = 1;
      tm2c_rpc_stats_send.aborts = tm2c_rpc_stats->aborts;
      for (target = 0; target < NUM_DSL_NODES; target++) 
	{
	  ssmp_send(dsl_nodes[target], (ssmp_msg_t *) data);
	}

      tm2c_rpc_stats_send.stats_type = 2;
      tm2c_rpc_stats_send.commits = tm2c_rpc_stats->commits;
      for (target = 0; target < NUM_DSL_NODES; target++) 
	{
	  ssmp_send(dsl_nodes[target], (ssmp_msg_t *) data);
	}

      tm2c_rpc_stats_send.stats_type = 3;
      tm2c_rpc_stats_send.max_retries = tm2c_rpc_stats->max_retries;
      for (target = 0; target < NUM_DSL_NODES; target++) 
	{
	  ssmp_send(dsl_nodes[target], (ssmp_msg_t *) data);
	}
    ...
    {
      tm2c_rpc_stats_send.stats_type = 4;
      tm2c_rpc_stats_send.aborts_war = tm2c_rpc_stats->aborts_war;
      int target;
      for (target = 0; target < NUM_DSL_NODES; target++) 
	{
	  ssmp_send(dsl_nodes[target], (ssmp_msg_t *) data);
      ...
      tm2c_rpc_stats_send.stats_type = 5;
      tm2c_rpc_stats_send.aborts_raw = tm2c_rpc_stats->aborts_raw;
      for (target = 0; target < NUM_DSL_NODES; target++) 
	{
	  ssmp_send(dsl_nodes[target], (ssmp_msg_t *) data);
	}

      tm2c_rpc_stats_send.stats_type = 6;
      tm2c_rpc_stats_send.aborts_waw = tm2c_rpc_stats->aborts_waw;
      for (target = 0; target < NUM_DSL_NODES; target++) 
	{
	  ssmp_send(dsl_nodes[target], (ssmp_msg_t *) data);
	}
